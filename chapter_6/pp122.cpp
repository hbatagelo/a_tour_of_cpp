...
template <typename Container> void algo([[maybe_unused]] Container &c) {
  std::vector<Value_type<Container>> vec; // Keep results here
}

template <typename Value> using String_map = std::map<std::string, Value>;
String_map<int> m; // m is a std::map<std::string, int>

////////////////////////////////////////////////////////////////////////////////
// Compile-time if
template <typename T> void update([[maybe_unused]] T &target) {
